...
<cfloop from="1" to="#Len(rawdata)#" index="c">
	...
	<cfif IsNumeric(thischar)>
		...
		<cfif thisNumber is 0><cfset position = c /></cfif>
		<cfset thisNumber = thisNumber * 10 + thischar />
	<cfelse>
		<cfif thisNumber gt 0>
			<cfset NumberArray[position] = thisNumber />
			<cfset thisNumber = 0 />
		</cfif>
		<cfif thischar is "*">
...
<cfloop from="1" to="#Len(SpecialArray)#" index="ThisGear">
	<cfif SpecialArray[ThisGear] is 1>
		<cfset col = ThisGear MOD 140 />
		<cfset startpos = ThisGear - col + 1 />
		<cfset endpos = startpos + 139 />
		<cfset GearParts = [] />
		<!--- loop through one line --->
		<cfloop from="#startpos - 140#" to="#endpos+140#" index="c">
            <cfif ArrayIsDefined(NumberArray,c)>
				<cfset startpart = c - 1 />
				<cfset endpart = c + Len(NumberArray[c]) />
				<cfset IsNext=false />
                <cfif ThisGear is startpart or ThisGear is endpart>
					<cfset IsNext=true />
				<cfelseif ThisGear gte startpart-140 and ThisGear lte endpart-140>
					<cfset IsNext=true />
				<cfelseif ThisGear gte startpart+140 and ThisGear lte endpart+140>
					<cfset IsNext=true />
				</cfif>
				<cfif IsNext>
					<cfset ArrayAppend(GearParts,NumberArray[c]) />
				</cfif>
			</cfif>
		</cfloop>
		<!--- check to see if there are two parts --->
		<cfif ArrayLen(GearParts) is 2>
		    <cfdump var="#GearParts#" />
			<cfset SumOfParts = SumOfParts + (GearParts[1] * GearParts[2]) />
		</cfif>
	</cfif>
</cfloop>
<!---
