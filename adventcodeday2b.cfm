...
<cfloop list="#rawdata#" index="thisline" delimiters="#chr(10)##chr(13)#">
		...
		<cfloop from="1" to="#ListLen(thisroll, " ")#" index="c">
			...
			<cfelse>
				...
				<cfif Left(Trim(ThisWord),3) is "red">
					<cfif StoredNumber gt MaxRed><cfset MaxRed = StoredNumber /></cfif>
				<cfelseif Left(Trim(ThisWord),5) is "green">
					<cfif StoredNumber gt MaxGreen><cfset MaxGreen = StoredNumber /></cfif>
				<cfelseif Left(Trim(ThisWord),4) is "blue">
					<cfif StoredNumber gt MaxBlue><cfset MaxBlue = StoredNumber /></cfif>
				<cfelse>
	...
	<!--- calculate the power of the Maxes --->
	<cfset LinePower = MaxRed * MaxGreen * MaxBlue />
	<cfset PowerSum = PowerSum + LinePower />
	<cfoutput>
	#ThisID# is #LinePower#
	<br>
	</cfoutput>
...
<cfoutput>#PowerSum# is the total of powers </cfoutput>