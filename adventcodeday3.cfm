...
<cfset rawdata= reReplace(rawdata,"\s","","ALL") />
<cfoutput>
#Len(rawdata)# and #Len(rawdata)/140#
...
<cfset thisNumber = 0 />
<cfset SumOfParts = 0 />
<cfloop from="1" to="#Len(rawdata)#" index="c">
	<cfset thischar = Mid(rawdata,c,1) />
	<cfif IsNumeric(thischar)>
		<cfset SpecialArray[c] = 0 />
		<cfif thisNumber is 0><cfset NumberStruct["position"] = c /></cfif>
		<cfset thisNumber = thisNumber * 10 + thischar />
	<cfelse>
		<cfif thisNumber gt 0>
			<cfset NumberStruct["number"] = thisNumber />
			<cfset thisNumber = 0 />
			<cfset ArrayAppend(NumberArray,NumberStruct) />
			<cfset NumberStruct = {} />
		</cfif>
		<cfif thischar is ".">
			<cfset SpecialArray[c] = 0 />
		<cfelse>
			<cfset SpecialArray[c] = 1 />
		</cfif>
	</cfif>
</cfloop>


<!--- I now have an array of numbers with their positions, and the position of every special character --->
<cfloop array="#NumberArray#" index="ThisNumberStruct">
	<cfset startpos = ThisNumberStruct.position - 1 />
	<cfset endpos = ThisNumberStruct.position + Len(ThisNumberStruct.number) />
	<cfset IsPart=false />
	<cfloop from="#startpos#" to="#endpos#" index="pos">
		<cfif pos lte ArrayLen(SpecialArray) and SpecialArray[pos] is 1>
			<cfset IsPart=true />
		<cfelseif pos gt 140 and SpecialArray[pos-140] is 1>
			<cfset IsPart=true />
		<cfelseif pos+140 lte ArrayLen(SpecialArray) and SpecialArray[pos+140] is 1>
			<cfset IsPart=true />
		</cfif>
	</cfloop>
	<cfif IsPart>
		<cfdump var="#ThisNumberStruct#" />
		<cfset SumOfParts = SumOfParts + ThisNumberStruct.number />
	</cfif>
</cfloop>
<!---
<cfdump var="#NumberArray#" />
<cfdump var="#SpecialArray#" />	 --->	#SumOfParts#
</cfoutput>