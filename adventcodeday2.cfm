<cfsavecontent variable="rawdata">
...
<cfset ColorLimits = {"red" : 12, "green" : 13, "blue" : 14} />
<cfset ValidIDs = 0 />
<cfset thisID = 0 />
<cfloop list="#rawdata#" index="thisline" delimiters="#chr(10)##chr(13)#">
	<cfset thisID = thisID + 1 />
	<cfset ValidLine = true />
	<cfset ShavedLine = Trim(ListGetAt(thisline,2,":")) />
	<cfloop list = "#ShavedLine#" index = "thisroll" delimiters=";">
		<!--- should be pattern of number and color. if this pattern breaks I want the code to break --->
		<cfset ExpectNumber = true />
		<cfloop from="1" to="#ListLen(thisroll, " ")#" index="c">
			<cfset thisWord = ListGetAt(thisroll, c, " ") />
			<cfif ExpectNumber>
				<cfset StoredNumber = thisWord />
				<cfset ExpectNumber = false />
			<cfelse>
				<!--- check color --->
				<cfif Left(Trim(ThisWord),3) is "red">
					<cfif StoredNumber gt ColorLimits.red><cfset ValidLine = false /></cfif>
				<cfelseif Left(Trim(ThisWord),5) is "green">
					<cfif StoredNumber gt ColorLimits.green><cfset ValidLine = false /></cfif>
				<cfelseif Left(Trim(ThisWord),4) is "blue">
					<cfif StoredNumber gt ColorLimits.blue><cfset ValidLine = false /></cfif>
				<cfelse>
					ERROR: corrupt data <cfoutput>#ThisWord#</cfoutput> <br><br>
				</cfif>
				<cfset ExpectNumber = true />
				<cfset StoredNumber = "notanumber" />
			</cfif>
		</cfloop>
	</cfloop>
	<!--- judge if this line qualifies --->
	<cfoutput>
	#thisID#
	<cfif ValidLine>
		<cfset ValidIDs = ValidIDs + thisID />is valid
	<cfelse>
	is not valid 
	</cfif>
	<br>
...
<cfoutput>#ValidIDs# is the total of IDs </cfoutput>